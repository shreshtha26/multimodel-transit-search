...
def selected_stages(config, stages=None, skip_stages=()):
    ...
    requested_stages = stages or STAGE_ORDER
    unknown_stages = [stage for stage in requested_stages if stage not in STAGE_SPECS]
    if unknown_stages:
        raise ValueError(f"Unknown experiment stages: {unknown_stages}")
    picked = [stage for stage in requested_stages if stage not in skipped and stage_config.get(stage, {}).get("enabled", True)]
    if not picked:
...
def stage_output_dir(base_output_dir, stage, config):
    ...
    output_subdir = stage_config.get("output_subdir", STAGE_SPECS[stage]["output_subdir"])
    return Path(base_output_dir) / str(output_subdir)
...
def load_stage(stage):
    ...
    if spec is None or spec.loader is None:
        raise ImportError(f"Could not load stage module from {script_path}")
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    if not hasattr(module, "main"):
        raise AttributeError(f"{script_path} does not define main(settings).")
    return module
...
def stage_output_paths(stage, output_dir, prefix):
    ...
    if stage == "gap_mode_comparison":
        return {"comparison_csv": str(metrics / f"{prefix}_gap_mode_comparison.csv"), "report_json": str(metrics / f"{prefix}_gap_mode_report.json"), "plot_manifest": str(metrics / f"{prefix}_gap_mode_plot_manifest.csv")}
    if stage == "gap_mode_injection":
        return {"summary_csv": str(metrics / f"{prefix}_gap_mode_injection_summary.csv"), "report_json": str(metrics / f"{prefix}_gap_mode_injection_report.json"), "results_csv": str(metrics / f"{prefix}_gap_mode_injection_results.csv")}
    if stage == "bls_baseline":
        return {"summary_json": str(metrics / f"{prefix}_bls_summary.json"), "periodogram_csv": str(metrics / f"{prefix}_bls_periodogram.csv"), "top_peaks_csv": str(metrics / f"{prefix}_bls_top_peaks.csv"), "null_trials_csv": str(metrics / f"{prefix}_bls_null_trials.csv"), "fap_thresholds_csv": str(metrics / f"{prefix}_bls_fap_thresholds.csv")}
    return {}
...
def summarize_stage(stage, output_dir, prefix):
    ...
    if stage == "single_target_arima":
        ...
        return {"phase1_engineering_complete": phase1.get("phase1_engineering_complete"), "phase1_scientific_ready_for_phase2": phase1.get("phase1_scientific_ready_for_phase2"), "selected_quality_policy": phase1.get("selected_quality_policy"), "selected_mode": phase1.get("selected_mode"), "selected_order": phase1.get("selected_order"), "n_injections": recovery.get("n_injections"), "rank1_recovery_rate": recovery.get("rank1_recovery_rate"), "rank3_recovery_rate": recovery.get("rank3_recovery_rate")}
    if stage == "gap_mode_comparison":
        report = read_json(metrics / f"{prefix}_gap_mode_report.json")
        best = report.get("best_available", {})
        return {"same_candidate_family_across_modes": report.get("same_candidate_family_across_modes"), "no_scientifically_acceptable_combination": report.get("no_scientifically_acceptable_combination"), "best_available_gap_mode": best.get("gap_mode"), "best_available_selected_order": best.get("selected_order"), "best_available_stationarity_conclusion": best.get("stationarity_conclusion"), "best_available_scientifically_acceptable": best.get("scientifically_acceptable")}
    if stage == "gap_mode_injection":
        report = read_json(metrics / f"{prefix}_gap_mode_injection_report.json")
        return {"injection_count": report.get("injection_count"), "best_median_snr_retention_mode": report.get("best_median_snr_retention_mode"), "highest_spurious_peak_rate_mode": report.get("highest_spurious_peak_rate_mode"), "any_mode_top_recovers_all_at_far_0.01": report.get("any_mode_top_recovers_all_at_far_0.01"), "any_mode_scientifically_acceptable_before_injection": report.get("any_mode_scientifically_acceptable_before_injection")}
    if stage == "bls_baseline":
        summary = read_json(metrics / f"{prefix}_bls_summary.json")
        return {"injected_period_days": summary.get("injected_period_days"), "recovered_period_days": summary.get("recovered_period_days"), "period_error_fraction": summary.get("period_error_fraction"), "period_matched": summary.get("period_matched"), "injected_beats_null": summary.get("injected_beats_null"), "injected_power": summary.get("injected_power"), "null_power": summary.get("null_power"), "empirical_fap": summary.get("empirical_fap"), "fap_1_percent_threshold": summary.get("fap_1_percent_threshold"), "fap_0_1_percent_threshold": summary.get("fap_0_1_percent_threshold"), "passes_fap_1_percent": summary.get("passes_fap_1_percent"), "passes_fap_0_1_percent": summary.get("passes_fap_0_1_percent"), "n_null_trials": summary.get("n_null_trials"), "null_block_size_cadences": summary.get("null_block_size_cadences")}
    return {}
...
def run_experiment(target_id=TARGET_ID, quarter=QUARTER, config_path=CONFIG_PATH, output_dir=OUTPUT_DIR, stages=STAGES, skip_stages=SKIP_STAGES, continue_on_error=CONTINUE_ON_ERROR, dry_run=DRY_RUN):
    ...
    for stage in selected_stages(config, stages, skip_stages):
        ...
        try:
            stage_module = load_stage(stage)
            return_code = int(stage_module.main(settings))
        except Exception as exc:
            return_code = 1
            record["stages"][stage] = {"output_dir": str(settings.output_dir), "return_code": return_code, "success": False, "runtime_seconds": float(time.perf_counter() - start), "outputs": outputs, "summary": {}, "error": f"{type(exc).__name__}: {exc}"}
        else:
            ok = return_code == 0
            record["stages"][stage] = {"output_dir": str(settings.output_dir), "return_code": return_code, "success": ok, "runtime_seconds": float(time.perf_counter() - start), "outputs": outputs, "summary": summarize_stage(stage, settings.output_dir, prefix) if ok else {}}
...
def main():
    ...
    prefix = target_prefix(TARGET_ID, QUARTER)
    record_path = project_path(OUTPUT_DIR) / "records" / f"{prefix}_experiment_record.json"
